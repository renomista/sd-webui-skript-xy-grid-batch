...
def load_batch_data():
	...
	data_path = os.path.join(modules.paths.script_path, "batch.json")
	print("%s" % data_path)
...
class Script(scripts.Script):
    ...
    def ui(self, is_img2img):
        ...
        batch_process = gr.Checkbox(label='Batch Process (load data from batch.json in sd-webui main directory)', value=False)
        include_lone_images = gr.Checkbox(label='Include Separate Images', value=False)
        no_fixed_seeds = gr.Checkbox(label='Keep -1 for seeds', value=False)
        

        return [x_type, x_values, y_type, y_values, draw_legend, batch_process, include_lone_images, no_fixed_seeds]
        
    def run(self, p, x_type, x_values, y_type, y_values, draw_legend, batch_process, include_lone_images, no_fixed_seeds):
    	...
    	if not batch_process:
    	    self.run2(p, x_type, x_values, y_type, y_values, draw_legend, include_lone_images, no_fixed_seeds)
    	else:
    	    batch_data = load_batch_data()
    	    for b in batch_data:
    	    	if "xvalues" in b:
    	    		new_xvalues = b["xvalues"]
    	    	else:
    	    		new_xvalues = x_values
    	    	if "yvalues" in b:
    	    		new_yvalues = b["yvalues"]
    	    	else:
    	    		new_yvalues = y_values
    	    	if "seed" in b:
    	    		p.seed = b["seed"]
    	    	else:
    	    		p.seed = orig_seed
    	    	if "prompt" in b:
    	    		...
    	    		p.prompt = orig_prompt
    	    	if "negative_prompt" in b:
    	    		p.negative_prompt = b["negative_prompt"]
    	    	else:
    	    		p.negative_prompt = orig_negative_prompt
    	    	self.run2(p, x_type, new_xvalues, y_type, new_yvalues, draw_legend, include_lone_images, no_fixed_seeds)

    def run2(self, p, x_type, x_values, y_type, y_values, draw_legend, include_lone_images, no_fixed_seeds):
